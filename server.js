...
app.get("/connect", function (request, response) {
  ...
  if(typeof request.query.access_token == 'undefined' || typeof request.query.username == 'undefined'){
    response.redirect('/');
  } else {
    ...
    var req = https.request(options, (res) => {
      res.on('data', function (chunk) {
        ...
        if(typeof body.goals != 'undefined' && body.goals.length != 0){
          ...
          response.sendFile(__dirname + '/views/no-goals.html');
        }
      });
    });
    req.on('error', (e) => {
...
app.get("/setup-link", (request, response) => {
  response.redirect('/');
});

app.get("/show-links", (request, response) => {
  if(typeof request.session.userId == 'undefined' || request.session.userId == null){
    response.redirect('/');
  } else {
    request.session.linktype = null;
    User.findOne({where: {id:request.session.userId}}).then( (user) => {
      console.log(user);
      UserGoal.findAll({where:{user_id:request.session.userId}}).then( (data) => {
        response.render(__dirname + '/views/show-links', {usergoals:data, user: user, connections: settings.connections});
      }, (err) => { console.log(err); response.redirect('/'); });
    }, (err) => { console.log(err); response.redirect('/'); });
  }
});
...
app.get("/delete-link", (request, response) => {
  if(typeof request.session.userId == 'undefined'){
    ...
    UserGoal.destroy({where: {user_id:request.session.userId, id: id}}).then( (data) => {
      response.redirect('/show-links');
    }, (err) => { console.log(err); response.redirect('/show-links'); });
  }
});

app.post("/setup-link", function (request, response) {
  if(typeof request.session.userId == 'undefined'){
    response.redirect('/');
  } else {
    if (!request.session.linktype && !request.body.linktype) {
      ...
      if(request.body.linktype && typeof settings.connections[request.body.linktype] == 'undefined'){
        response.redirect('/show-links');
      ...
      User.findOne({where:{id:request.session.userId}}).then( (user) => {
        ...
        var linktype = request.session.linktype ? request.session.linktype : request.body.linktype;
        ...
        getBeeminderGoals(user, (goals) => {
          if(!goal){
            response.render(__dirname + '/views/setup-link-'+linktype, {user: user, goals:goals, params: params, start_value: null, score:null});
          } else {
            getBeeminderGoalValue(user, goal, (curval) => {
              if(start_value < curval) {start_value = curval}
              getSourceValue({linktype: linktype, params: JSON.stringify(params)}, (score) => {
                if(goal && linktype && typeof start_value != 'undefined' && typeof score != 'undefined' && request.body.submit == 'Start tracking'){
                  updateBeeminderGoal({goal:goal, linktype: linktype, params: JSON.stringify(params), start_value: start_value, score:score}, user, () => {response.redirect('/show-links');});
                } else {
                  console.log('Here1');
                  response.render(__dirname + '/views/setup-link-'+linktype, {user: user, goal:goal, goals:null, params: params, start_value: start_value, score:score});
                }
              }, (err) => {
                response.render(__dirname + '/views/setup-link-'+linktype, {user: user, goal:goal, goals:goals, params: params, start_value: start_value, score:null, source_error: err});
              });
            }, (err) => {
              response.render(__dirname + '/views/setup-link-'+linktype, {user: user, goal:goal, goals:goals, params: params, start_value: start_value, score:null, beeminder_error: err});
            });
          }
        }, () => { response.sendFile(__dirname + '/views/no-goals.html'); });
      }, () => { response.redirect('/'); });
...
function updateBeeminderGoal(data, user, callback=()=>{}){
  UserGoal.findOrCreate({where: {user_id:user.id, goal:data.goal}}).spread((usergoal, created) => {
    if(created){
    ...
    var options = {
      host: 'www.beeminder.com',
      port: 443,
      path: '/api/v1/users/' + user.username + '/goals/' + data.goal + '/datapoints.json?access_token=' + user.access_token + '&value=' + ( data.score*1 - data.start_value*1 ),
      method: 'POST',
      body: ''
    };
    var req = https.request(options, (res) => {
      ...
      res.on('data', function (chunk) {
        ...
        UserGoal.update({score:data.score, last_updated: Date.now()},{where: {id:usergoal.id}}).then(callback());
      });
    });
    req.on('error', (e) => {
      ...
      User.findOne({where: {id:user.id}}).then( (user) => {
        getBeeminderGoalValue (user, usergoal.goal, (curval) => {
          getSourceValue (usergoal, (sourceval) => {
            if(curval < sourceval) {
              updateBeeminderGoal({goal:usergoal.goal, linktype: usergoal.linktype, params: usergoal.params, start_value: curval, score: sourceval}, user);
            }
          }); 
        })
      });
...
function getBeeminderGoalValue (user, goal, callback, error_callback = ()=>{}) {
  var options = {
      host: 'www.beeminder.com',
      port: 443,
      path: '/api/v1/users/' + user.username + '/goals/' + goal + '.json?access_token=' + user.access_token,
      method: 'GET'
    };
...
function getSourceValue (goal, callback, error_callback = ()=>{}) {
  ...
  if (typeof settings.connections[goal.linktype] == 'undefined') { return false; }

  var link = parseLink(settings.connections[goal.linktype]['href'], params);
  var re = /^(?:http|https):\/\/\S*\.\S*/i;
  if(!re.test(link)) { return false; }
  
  if(link.substr(0,6)=='https:'){
    var http_transport = https;
  } else {
    var http_transport = http;
  }
